	...
	public static function get_icl_translator_status() {
		...
		if ( isset( $website_details['translation_languages']['translation_language'] ) ) {
			...
			foreach ( $translation_languages as $lang ) {
				...
				$language_pairs[] = array(
					'from'                  => $sitepress->get_language_code( WPML_Pro_Translation::server_languages_map( $lang['attr']['from_language_name'], true ) ),
					'to'                    => $sitepress->get_language_code( WPML_Pro_Translation::server_languages_map( $lang['attr']['to_language_name'], true ) ),
