...
class GFML_TM_API extends Gravity_Forms_Multilingual {
	...
	public function gform_post_update_form_meta_action( $form_meta, $form_id, $meta_name) {
	...
	protected function register_gf_string( $string_value, $string_name, $package, $string_title, $string_kind = 'LINE' ) {
		do_action( 'wpml_register_string', $string_value, $string_name, $package, $string_title, $string_kind );
	}

	protected function register_strings_common_fields( $form_field, $form_package, $form ) {
		...
		foreach ( $this->form_fields as $form_field_key ) {
			if ( ! empty( $form_field->{$form_field_key} ) && $form_field->type !== 'page' ) {
				...
				$this->register_placeholders( $form_package, $form_field, $form );
				$this->register_customLabels( $form_package, $form_field, $form );
			}
		}
	}
	...
	protected function register_strings_fields( $form, $form_package ) {
		...
		foreach ( $form[ 'fields' ] as $field_id => $form_field ) {
			...
			$this->register_strings_common_fields( $form_field, $form_package, $form );
	...
	public function register_missing_packages($type = null, $existing = null) {
		global $wpdb;
