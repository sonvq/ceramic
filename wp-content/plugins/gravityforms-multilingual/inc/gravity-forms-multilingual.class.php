...
abstract class Gravity_Forms_Multilingual {
	...
	function get_document_view_link( $post_view_link, $label, $current_document, $element_type, $content_type ) {
		if ( 'package' == $element_type && 'gravity_form' === $content_type ) {
			...
			if ( $element_id ) {
				...
				if ( $form_id ) {
					$post_view_link = sprintf( '<a href="%s" target="_blank">%s</a>', '/?gf_page=preview&id=' . $form_id, $label );
				}
			}
		}
					...
					if ( in_array( $field[ 'inputType' ], array( 'singleproduct', 'singleshipping' ) ) && isset( $field[ 'basePrice' ] ) ) {
						...
						foreach ( $field[ 'choices' ] as $index => $choice ) {
							...
							if ( isset( $choice[ 'price' ] ) ) {
								$string_data[ $snh->get_field_multi_input_choice_price() ] = $choice[ 'price' ];
							}
						}
					}
					break;
			...
			switch ( $field[ 'type' ] ) {
				...
				case 'list':
					if ( is_array( $field->choices ) ) {
						foreach ( $field->choices as $index => $choice ) {
							...
							if ( isset( $choice[ 'price' ] ) ) {
								$string_data[ $snh->get_field_multi_input_choice_price() ] = $field->choices[ $index ][ 'price' ];
							}
						}
					}
					break;
				...
				case 'option':
					...
					if ( in_array( $field->inputType, array(
						...
						$string_data[ $snh->get_field_option_basePrice() ] = $field->basePrice;
		...
		foreach ( $form[ 'fields' ] as $id => $field ) {
			...
			switch ( $field[ 'type' ] ) {
				...
				case 'option':
					// Price fields can be single or multi-option field type
					if ( in_array( $field[ 'inputType' ], array( 'singleproduct', 'singleshipping' ) ) && isset( $field[ 'basePrice' ] ) ) {
						$form[ 'fields' ][ $id ][ 'basePrice' ] = icl_t( $st_context, $snh->get_field_option_basePrice(), $field[ 'basePrice' ] );
					} else if ( in_array( $field[ 'inputType' ], array( 'select', 'checkbox', 'radio', 'hiddenproduct' ) ) && ! empty( $field[ 'choices' ] ) ) {
						foreach ( $field[ 'choices' ] as $index => $choice ) {
							$snh->field_choice                                        = $choice;
							...
							$form[ 'fields' ][ $id ][ 'choices' ][ $index ][ 'text' ] = $translation;
							if ( isset( $choice[ 'price' ] ) ) {
								$translation                                               = icl_t( $st_context, $snh->get_field_multi_input_choice_price(), $choice[ 'price' ] );
	...
	function get_form( $form_id, $lang = null ) {
		...
		return isset( $this->_current_forms[ $form_id ][ $lang ] ) ? $this->_current_forms[ $form_id ][ $lang ] : $this->gform_pre_render( RGFormsModel::get_form_meta( $form_id ), false );
	}
